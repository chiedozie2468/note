...
const initialTasks: TaskItem[] = [
  {
    ...
    description:
      "Build adaptive logic layers to parse client parameters across telemetry middleware. Ensure type contracts stay perfectly uniform during production loads.",
    status: "In Progress",
    priority: "Critical",
    ...
    subTasks: [
      {
        id: "sub-1",
        title: "Map out full security configuration parameters",
        isCompleted: true,
      },
      {
        id: "sub-2",
        title: "Inject middleware logging handlers across routers",
        isCompleted: false,
      },
      {
        id: "sub-3",
        title: "Execute comprehensive mutation suite locally",
        isCompleted: false,
      },
    ],
    comments: [
      {
        id: "c-1",
        author: "Daniel 70",
        text: "The security configurations are fully aligned. Pushing updates.",
        timestamp: "2026-06-23T09:12:00.000Z",
      },
      {
        id: "c-2",
        author: "Chiedozie Onyekwelu",
        text: "Excellent work. Let's aim to have the mutation test suite wrapped up before review.",
    ...
    activityLog: [
      {
        ...
        actor: "System Core",
        action: "Task instantiated automatically via sprint scheduler.",
      },
      {
        id: "a-2",
        timestamp: "2026-06-23T09:00:00.000Z",
        actor: "Daniel 70",
        action: "Updated state configuration parameters.",
      },
    ],
    dueDate: "2026-06-26",
  ...
  },
  {
    ...
    description:
      "Refactor spatial indices and electricity reliability matrix aggregates for high-speed map responses.",
    status: "Todo",
    priority: "High",
    ...
    subTasks: [
      {
        id: "sub-4",
        title:
          "Refactor nested hooks optimized for high-density spatial arrays",
        isCompleted: true,
      },
    ],
    comments: [],
    activityLog: [
      {
        ...
        action: "Created task workspace target.",
      },
    ],
    dueDate: "2026-06-29",
...
export default function PremiumWorkspace() {
  ...
  const [activeFilter, setActiveFilter] = useState<
    "All" | "My Tasks" | "Critical" | "Completed"
  >("All");
  ...
  const filteredTasks = useMemo(() => {
    return tasks.filter((t) => {
      ...
      if (activeFilter === "My Tasks")
        return t.assignees.includes("Chiedozie Onyekwelu");
          ...
          {[
            {
              label: "Active Queue",
              count: metrics.total,
              description: "Total tracked epics",
              color: "text-white",
            },
            {
              label: "In Production",
              count: metrics.completed,
              description: "Completed pipelines",
            ...
            {
              label: "Critical Halts",
              count: metrics.critical,
              description: "Requires action immediately",
              color: "text-rose-400",
            },
          ].map((card, idx) => (
            <div
          ...
          <div className="space-y-4 lg:col-span-7 xl:col-span-8">
            ...
            <div className="flex flex-wrap items-center justify-between gap-3 border-b border-white/5 pb-4">
              <div className="flex items-center gap-1.5 rounded-xl bg-[#0B1423] p-1 border border-white/5">
                {(["All", "My Tasks", "Critical", "Completed"] as const).map(
                  (opt) => (
                    <button
                      key={opt}
              ...
              {filteredTasks.length === 0 ? (
                <div className="flex flex-col items-center justify-center rounded-2xl border border-dashed border-white/10 py-16 text-center">
                  ...
                  <p className="mt-3 text-sm font-medium text-slate-400">
                    No core metrics match execution criteria.
                  </p>
                  <p className="text-xs text-slate-500">
                    Adjust the filtering parameters or instantiate an objective.
                  </p>
                </div>
              ) : (
                filteredTasks.map((task) => {
                  const doneSub = task.subTasks.filter(
                    (s) => s.isCompleted,
                  ).length;
                  const totalSub = task.subTasks.length;
                  const percentage =
                    totalSub > 0 ? Math.round((doneSub / totalSub) * 100) : 0;
                      ...
                      {task.teams.length > 0 && (
                        <div className="mt-3 flex flex-wrap gap-1.5">
                          {task.teams.map((tName, i) => {
                            const config = initialTeams.find(
                              (it) => it.name === tName,
                            );
                            return (
                              <span
          ...
          <div className="lg:col-span-5 xl:col-span-4">
            {activeTask ? (
              <div className="sticky top-24 rounded-2xl border border-white/[0.06] bg-white/[0.01] p-5 shadow-xl shadow-black/10 backdrop-blur-md space-y-6">
                ...
                <div>
                  <div className="flex items-center justify-between gap-2 text-xs">
                    ...
                    <span className="text-slate-400">
                      Created by {activeTask.createdBy}
                    </span>
                  </div>
                ...
                <div className="space-y-2 rounded-xl bg-[#0B1423] p-3 border border-white/5">
                  <label className="text-[10px] font-bold uppercase tracking-wider text-slate-400 flex items-center gap-1">
                    <TrendingUp className="h-3 w-3 text-cyan-400" /> Lifecycle
                    Matrix Route
                  </label>
                  <div className="grid grid-cols-2 gap-1.5 pt-1">
                    {(
                      ...
                      <button
                        key={statusOption}
                        onClick={() =>
                          handleUpdateStatus(activeTask.id, statusOption)
                        }
                ...
                <div className="space-y-1.5">
                  ...
                  <p className="text-xs leading-relaxed text-slate-300 bg-white/[0.02] border border-white/5 p-3 rounded-xl">
                    {activeTask.description ||
                      "No operational instructions provided."}
                  </p>
                </div>
                ...
                <div className="space-y-3">
                  ...
                  <div className="space-y-1.5">
                    {activeTask.subTasks.map((st) => (
                      <div
                        key={st.id}
                        onClick={() =>
                          handleToggleSubTask(activeTask.id, st.id)
                        }
                  ...
                  <div className="flex gap-2">
                    <input
                      ...
                      onKeyDown={(e) =>
                        e.key === "Enter" && handleAddSubTask(activeTask.id)
                      }
                ...
                <div className="space-y-3 border-t border-white/5 pt-4">
                  ...
                  <div className="max-h-44 space-y-2.5 overflow-y-auto pr-1">
                    {activeTask.comments.length === 0 ? (
                      <p className="text-[11px] text-slate-500 italic">
                        No sync metrics found on thread.
                      </p>
                    ) : (
                      activeTask.comments.map((c) => (
                        <div
                          key={c.id}
                          className="rounded-xl bg-[#0B1423]/60 p-2.5 border border-white/[0.04]"
                        >
                  ...
                  <div className="flex gap-2">
                    <input
                      ...
                      onKeyDown={(e) =>
                        e.key === "Enter" && handleAddComment(activeTask.id)
                      }
                      className="flex-1 rounded-xl border border-white/10 bg-[#0B1423] px-3 py-1.5 text-xs text-white outline-none transition focus:border-cyan-400/50"
                    />
                    <button
              ...
              <div className="rounded-2xl border border-dashed border-white/10 p-8 text-center text-xs text-slate-500">
                Select an active objective queue block to display runtime
                parameters.
              </div>
            )}
          </div>
            ...
            <form onSubmit={handleAddTask} className="space-y-4">
              ...
              <div className="space-y-1.5">
                ...
                <div className="flex flex-wrap gap-2">
                  {["Frontend Team", "Backend Team", "Design Operations"].map(
                    (t) => {
                      ...
                      return (
                        <button
                          ...
                          key={t}
                          onClick={() =>
                            toggleFormSelection(t, formTeams, setFormTeams)
                          }
